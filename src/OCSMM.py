...
class OCSMM(DM):
    ...
    Octo-Voxel Contact Surface Multi Manager (OCSMM) - Asynchronous approaches and multiprocessing are employed in a class created
    ...
    def process_octovoxel(self, 
        ...
        Tuple[np.ndarray, int]
            A tuple containing an array of Combinations_int and Contact_surface value.

        Notes:
        ------
        - Octovoxel size is set to 2.
        - Contact_surface value is the sum of Combinations_int.
        ...
        print(f"Processing file... {File_path} ----------- {Contact_surface}");

        return Combinations_int, Contact_surface;

    @Timer.timer("Execution_OCSMM.log")
    ...
    def get_array(self, Depth: int, Height: int, Width: int) -> None:
        ...
        - Combinations_int and Contact_surface values are accumulated across all processed files.
        - The results are saved to a CSV file using the `save_to_csv_Contact_surface` method.
        ...
        if self.Folder:
            ...
            with multiprocessing.Pool(processes=self.Num_processes) as pool:
                ...
                Combination, Contact_surface = zip(*Data);

                '''Result_combination = np.sum(Combination, axis=0);
                Result_Contact_surface = np.sum(Contact_surface, axis=0);'''
                
                for _, (C, E) in enumerate(zip(Combination, Contact_surface)):

                    Combinations_all.append(C);
                    Contact_surface_all.append(E);
